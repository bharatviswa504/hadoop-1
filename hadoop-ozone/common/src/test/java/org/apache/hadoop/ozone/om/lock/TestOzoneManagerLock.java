...
public class TestOzoneManagerLock {
  ...
  public void reacquireResourceLock() {
    ...
    for (OzoneManagerLock.Resource resource :
        OzoneManagerLock.Resource.values()) {
      if (resource == OzoneManagerLock.Resource.BUCKET) {
        resourceName = OzoneManagerLockUtil.generateResourceLockName(resource,
            UUID.randomUUID().toString(), UUID.randomUUID().toString());
      } else {
        resourceName =
            OzoneManagerLockUtil.generateResourceLockName(resource,
                resourceName);
      }
  ...
  public void releaseLockWithOutAcquiringLock() {
    String userLock =
        OzoneManagerLockUtil.generateResourceLockName(OzoneManagerLock.Resource.USER, "user3");
