...
public class OMPrefixRemoveAclRequest extends OMPrefixAclRequest {
  ...
  private static QuadFunction<OzoneObj, List<OzoneAcl>, OmPrefixInfo,
      PrefixManagerImpl, OMPrefixAclOpResult, IOException> prefixRemoveAclOp;
  private OzoneObj ozoneObj;
  private List<OzoneAcl> ozoneAcls;


  static {
    prefixRemoveAclOp = (ozoneObj, ozoneAcls, omPrefixInfo, prefixManager) -> {
      return prefixManager.removeAcl(ozoneObj, ozoneAcls.get(0), omPrefixInfo);
    };
  }

  public OMPrefixRemoveAclRequest(OMRequest omRequest) {
    super(omRequest, prefixRemoveAclOp);
  ...
  void onComplete(boolean operationResult, IOException exception,
      OMMetrics omMetrics) {
    if (operationResult) {
      LOG.debug("Remove acl: {} to path: {} success!", getAcls(),
          getOzoneObj().getPath());
    } else {
      omMetrics.incNumBucketUpdateFails();
      if (exception == null) {
        LOG.error("Remove acl {} to path {} failed, because acl doesnot exist",
            getAcls(), getOzoneObj().getPath());
      } else {
        LOG.error("Remove acl {} to path {} failed!", getAcls(),
