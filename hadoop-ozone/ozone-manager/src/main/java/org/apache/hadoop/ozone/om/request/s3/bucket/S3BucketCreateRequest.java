...
public class S3BucketCreateRequest extends OMVolumeRequest {
    ...
    try {
      ...
      try {
        ...
        if (!omMetadataManager.getVolumeTable().isExist(volumeKey)) {
          ...
          createVolume(omMetadataManager, omVolumeArgs, volumeList, volumeName,
              userName, transactionLogIndex);
          volumeCreated = true;
        }
      } finally {
        if (acquiredUserLock) {
   ...
   * @return s3Mapping
   */
  private String formatS3MappingName(String volumeName, String bucketName) {
