...
public abstract class OMClientResponse {
   ...
   * buffer and add response to cache and cannot return, as when flush is missed
   * in HA, ratis has provided guaranty to apply the transactions again. In
   * Non-HA, we cannot use the same model, so we need to apply response to OM
   * DB and then return response.
   */
